...
This project uses a GitHub Actions–based CI/CD pipeline to continuously validate and package the service. The workflow is defined in `.github/workflows/ci.yml` and is triggered on every push and every pull request targeting the `main` or `master` branches. At a high level, the pipeline enforces code quality and consistency, runs automated tests against the API, and builds a production-grade Docker image from `docker/Dockerfile.prod`, ensuring that changes are always in a deployable state.
